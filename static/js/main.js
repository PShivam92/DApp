...
function newWallet() {
  ...
  var randomSeed = lightwallet.keystore.generateRandomSeed();
      $('.modal-content #ranseed').text(randomSeed);
      $('#ranseed1').val(randomSeed);
  $("#myModal2").modal('show');
...
function newAddresses(password) {
  ...
  var numAddr = 3;
    ...
    console.log("new address  " + ter)
    prv_key = global_keystore.exportPrivateKey(ter, pwDerivedKey);
    console.log("private key in newAddresses" + prv_key);
    document.getElementById('sendFrom').value = ter;
    document.getElementById('pvt_key').value = prv_key;
    document.getElementById('sendFrom1').value = ter;
    document.getElementById('pvt_key1').value = prv_key;
  })
};
...
tokenContract.balancesof(account,function(err, res){
    if(!err){
      ...
      $("#tknbal3").html(JSON.parse(res));
      $("#loginform #tokenbal2").val(JSON.parse(res));
    }
    else{
      console.log("error");
    }
...
tokenContract.balancesof(owner_account,function(err, res){
    if(!err){
      $("#tokens_sold").html(1000000-JSON.parse(res));    
    }
    else{
      console.log("error");
    }
...
function ch_checkethBal(account)
{
	var account = $("#sendFrom").val();
   var token = web3.eth.contract(tokenabi);
...
{   
  str='#0tkn';
res1 = str.replace("0", i);
 str1='#0eth';
res2 = str1.replace("0", i);
var account1 = $("#"+i).text();
checkEtherBalance1(account1,res2)
checkTokenBalance1(account1,res1);
}


 
function  checkEtherBalance1(account1,res1){
  var x=web3.eth.getBalance(account1, function(error, result){
       if(!error){
          var ethbal=JSON.parse(web3.fromWei(result.toFixed(2),'ether'));
           $(res1).html(ethbal.toFixed(2));
       }else{
        $(res1).html(err);
          console.log("error");
           console.error("error");
...
function checkTokenBalance1(account1,res1){
  tokenContract.balancesof(account1,function(err, res){
    if(!err){
      $(res1).html(JSON.parse(res)+" ETGCN");
    }
    else{
      $(res1).html(err);
      console.log("error");
...
$("#checkGoalReached").click(function(){
  myContract.checkGoalReached(function(err,res){
      if(!err){

          console.log(res+"successfully sent ether to beneficiary account");

      }
      else{
        console.log("crowdsale is closed and crowdsaleClosed = true");
      }
  })
});
...
filter.get(function(error, result){
 ...
 web3.eth.getBlock("4878672",function(error,data){
            if (error) {
 ...
 web3.eth.listTransactions("0xc9d6234e91d3c0FfF822bb767E927396c9160AA6",function(error,data){
            if (error) {
